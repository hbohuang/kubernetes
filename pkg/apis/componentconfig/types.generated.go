		...
		if false {
			...
			if yyr2 || yy2arr2 {
				...
				if false {
					...
					r.EncodeString(codecSelferC_UTF81234, string(x.CAdvisorDbDriver))
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
				r.EncodeString(codecSelferC_UTF81234, string("cAdvisorDbDriver"))
				z.EncSendContainerState(codecSelfer_containerMapValue1234)
				yym103 := z.EncBinary()
				_ = yym103
				if false {
...
func (x *KubeletConfiguration) codecDecodeSelfFromMap(l int, d *codec1978.Decoder) {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "cAdvisorDbDriver":
			if r.TryDecodeAsNil() {
				x.CAdvisorDbDriver = ""
			} else {
				x.CAdvisorDbDriver = string(r.DecodeString())
			}
		case "healthzPort":
			if r.TryDecodeAsNil() {
				x.HealthzPort = 0
